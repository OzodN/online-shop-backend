...
- Completed S3/T2: User, Role, RefreshToken Entities & Repositories. Key design decisions: (1) `Role` does NOT extend `BaseEntity` — standalone entity with only `id` + `name` to match the Flyway schema (no audit columns), (2) `RefreshToken` does NOT extend `BaseEntity` — standalone with selective `@CreatedDate` for `createdAt` only. `User` extends `SoftDeletableEntity` correctly. All `UserRepository` queries filter soft-deleted users (`deletedAt IS NULL`). Good use of `@Builder.Default` on `roles` and `revoked` fields. Review approved on first round — all 12/12 acceptance criteria met. Ready for S3/T3.
